...
class ResponseValueMapper(private val scalarRegistry: ScalarTypeRegistry) {
    ...
    fun toValue(value: Any?): Value =
        when {
            ...
            value is Iterable<*> && value !is Path -> {
                Value.ListValue(items = value.map { item: Any? -> toValue(item) })
            }

            value::class.java.isArray -> Value.ListValue(items = reflectArray(value))

            value is Enum<*> -> Value.Scalar(ScalarValue.StringValue(value.name))

            else -> encodeScalar(value) ?: toRecord(value)
        }
